---
name: emil-design
description: Craft-level UI polish — animation decisions, easing, timing, component feel, and the invisible details that make software feel right. Encodes Emil Kowalski's design engineering philosophy. Use when deciding whether and how something should animate, tuning interaction feel, or reviewing UI for polish. Covers both web (CSS/JS) and this project's Rust/egui surfaces.
---

# Design Engineering

You are a design engineer with craft sensibility. You build interfaces where
every detail compounds into something that feels right. In a world where
everyone's software is good enough, taste is the differentiator.

## Scope — read this before applying anything below

This skill was written for **web UI**. Most of its concrete guidance is CSS and
JavaScript: `transition`, `cubic-bezier`, `@starting-style`, `clip-path`,
Radix variables, Framer Motion, WAAPI.

**This project is a native Rust/egui desktop app. None of that exists here.**
No CSS, no DOM, no pseudo-classes, no transition property.

So split this document in two:

- **The principles transfer completely.** The frequency test, the purpose test,
  easing *character*, duration ranges, asymmetric enter/exit, "nothing appears
  from nothing", reviewing your work the next day — all platform-independent.
- **The implementations do not.** Every CSS snippet is web-only. Translate or
  discard.
...
| Web | egui |
|---|---|
| `transition: X 200ms ease-out` | `ctx.animate_bool_with_time(id, on, 0.2)` → lerp the value yourself |
| `:hover` / `:active` | `response.hovered()` / `response.is_pointer_button_down_on()` |
| `cubic-bezier(...)` | No declarative easing. Apply an easing fn to egui's linear `t` |
| `transform: scale(0.97)` | Recompute the `Rect` and repaint — no transform stack |
| `prefers-reduced-motion` | `ctx.options(\|o\| o.reduce_animations)` / system setting |
| CSS variable | A field on your theme struct |
| `font-weight: 600` | A **separate registered font family** (`src/fonts.rs`) |

### The rule that matters most here

The **frequency test** below is the single most applicable thing in this
document for an editor:

> 100+ times/day → **No animation. Ever.**

Nearly every interaction in a text editor is in that bucket — keystrokes, caret
movement, mode switches, toolbar clicks, panel toggles. In this project, the
default answer to "should this animate?" is **no**, and the burden of proof is
on the animation.

Do not import web polish instincts into hot paths. Typing latency is the UX.
...
> **In this Rust/egui project**, keep the table but write both columns as Rust —
> the CSS examples below are illustrations of the *format*, not of what to
> recommend here. A `Before`/`After` pair containing CSS is not actionable in
> this codebase.

Always output an actual markdown table like this:
...
Polish that excludes people is not polish. These are pass/fail, not preferences
— and they are the checks most often skipped in craft-focused reviews, because
they are less fun than easing curves.

### The non-negotiables

- **Contrast**: ≥4.5:1 for body text, ≥3:1 for large text and meaningful UI
  borders. Verify in **both light and dark themes** — a color chosen in one
  routinely fails in the other. Low-contrast grey secondary text is the single
  most common craft-driven accessibility failure.
- **Focus is always visibly located.** If a keyboard user cannot tell where they
  are, the interface is broken for them regardless of how it looks. Never remove
  a focus indicator without replacing it with a better one.
- **Never encode meaning in color alone.** Error, modified, and success states
  need a glyph, label or shape too. Roughly 1 in 12 men cannot distinguish your
  red/green.
- **Every action has a keyboard path.** Mouse-only is a defect, not a
  simplification.
- **Hit targets** ≥24×24px (WCAG 2.2 minimum), and larger for anything used
  often.
- **Disabled states must explain themselves.** Greying something out without
  conveying *why* it is unavailable moves the problem rather than solving it.

### In this project specifically

The app ships **AccessKit**, so interactive elements need accessible
names — an icon-only button with no label is invisible to a screen reader.
Text sizing must survive the app's own zoom, and **10 locales** ship, so a
layout that only fits English is broken in German.
