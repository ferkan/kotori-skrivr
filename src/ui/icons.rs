...
/// Named egui font family for the bundled Skrivr editor icons.
pub const SKRIVR_FONT_FAMILY: &str = "skrivr-icons";
...
/// Build a [`FontId`] for rendering a Skrivr icon glyph.
pub fn skrivr_font(size: f32) -> FontId {
    FontId::new(size, FontFamily::Name(SKRIVR_FONT_FAMILY.into()))
}

/// Build [`RichText`] for a Skrivr icon at the given size.
///
/// Glyphs are fitted to a uniform em square, so unlike the Phosphor set they
/// need no per-icon size nudging to sit evenly in square button slots.
pub fn skrivr_rich_text(icon: &str, size: f32) -> RichText {
    RichText::new(icon).font(skrivr_font(size))
}
