...
impl FileTreePanel {
    ...
    /// * `active_file` - Path of the file currently open in the editor, if any
    pub fn show(
        &mut self,
        ui: &mut egui::Ui,
        ...
        // Canonicalize once up front rather than per row per frame; row
        // comparisons fall back to a cheap `==` first and only canonicalize
        // the node path if that fails (see `render_tree_node`).
        let active_file_canonical = active_file.and_then(|p| p.canonicalize().ok());
            ...
            .show_inside(ui, |ui| {
                ...
                ui.allocate_ui_with_layout(
                    ...
                    |ui| {
                        ...
                        ui.add(
                            egui::Label::new(RichText::new(workspace_name).font(crate::fonts::chrome_bold_font(12.0)))
                                .truncate(),
                        );
                ...
                egui::ScrollArea::vertical()
                    ...
                    .show(ui, |ui| {
                        ...
                        self.render_tree_node(
                            ...
                            git_statuses,
                            active_file,
                            active_file_canonical.as_deref(),
                        );
                        ui.add_space(4.0);
                    });
    ...
    fn render_tree_node(
        ...
        git_statuses: Option<&HashMap<PathBuf, GitFileStatus>>,
        active_file: Option<&Path>,
        active_file_canonical: Option<&Path>,
    ) {
        ...
        // This row is selected when it is a file whose path matches the
        // active editor tab. Compare canonical paths where available
        // (cheap `==` first, canonicalizing this node's path only on
        // failure of that) since `active_file` was already canonicalized
        // once by the caller.
        let is_selected = !is_dir
            && active_file.is_some_and(|active| {
                if node.path == active {
                    return true;
                }
                match (active_file_canonical, node.path.canonicalize().ok()) {
                    (Some(active_canon), Some(node_canon)) => active_canon == node_canon,
                    _ => false,
                }
            });
        ...
        // Paint order: selection fill, then hover fill, then the selection
        // accent bar (so it survives being drawn over by hover), then text.
        if is_selected {
            ui.painter().rect_filled(row_rect, 2.0, selected_bg);
        }

        // Tree rows take clicks, so they can hold focus — without a ring a
        // keyboard user cannot tell which row is current.
        crate::ui::a11y::focus_ring(ui, &row_response, 2);
        ...
        if is_selected {
            let accent_bar = egui::Rect::from_min_max(
                row_rect.left_top() + Vec2::new(1.0, 1.0),
                row_rect.left_bottom() + Vec2::new(3.0, -1.0),
            );
            ui.painter()
                .rect_filled(accent_bar, 0.0, ui.visuals().selection.bg_fill);
        }
