//! Configuration file persistence for Kotori Skrivr
//!
...
const APP_NAME: &str = "kotori-skrivr";
...
///    - **Windows**: `%APPDATA%\kotori-skrivr\`
///    - **macOS**: `~/Library/Application Support/kotori-skrivr/`
///    - **Linux**: `~/.config/kotori-skrivr/`
///
/// # Errors
///
...
fn load_config_internal() -> Result<Settings> {
    ...
    // Bring older configs up to the current schema before anything reads them.
    let migrated = settings.migrate();
    if migrated {
        info!(
            "Migrated config at {} to schema v{}",
            config_path.display(),
            crate::config::settings::CURRENT_CONFIG_VERSION
        );
    }
    ...
    if migrated || pruned_files > 0 || pruned_workspaces > 0 {
...
mod tests {
    ...
    fn test_app_name_constant() {
        assert_eq!(APP_NAME, "kotori-skrivr");
    }
