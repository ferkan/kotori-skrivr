...
pub enum MaxLineWidth {
    /// No width limit
    Off,
    /// 80 characters (traditional terminal width) - default
    ///
    /// The reading measure. 66 characters is the classic single-column ideal,
    /// but this is an editor where lines are also markdown source, tables and
    /// fenced code, which wrap constantly below 80.
    #[default]
    #[serde(rename = "80")]
    Col80,
...
#[serde(rename_all = "lowercase")]
pub enum EditorFont {
    /// Literata - Serif designed for long-form reading (default editor body font)
    #[default]
    Literata,
...
impl EditorFont {
    ...
    pub fn description(&self) -> &'static str {
        match self {
            EditorFont::Literata => "Serif designed for long-form reading (default)",
    ...
    ///
    /// Used to pick the average character advance when converting a column
    /// count (e.g. [`MaxLineWidth::Col80`]) into pixels: a monospace advance is
    /// ~0.6em, a proportional one ~0.5em. Getting this wrong stretches the
    /// reading measure by ~20%.
    ///
    /// Custom system fonts are matched by name, since the family is resolved
    /// by the platform and its metrics are not known here. The heuristic errs
    /// toward proportional, which yields the narrower (safer) measure.
    pub fn is_monospace(&self) -> bool {
        match self {
            EditorFont::Literata => false,
            EditorFont::Inter => false,
            EditorFont::JetBrainsMono => true,
            EditorFont::Custom(name) => {
                let name = name.to_lowercase();
                ["mono", "code", "consol", "courier", "typewriter"]
                    .iter()
...
/// - `LiveMarkdown`: One continuous editing surface where markdown renders
///   styled as you type, with syntax markers hidden except on the cursor's
///   line (Typora-style). See `.claude/skills/livemd/SKILL.md`. Distinct from
///   `Rendered`, which is upstream's block-level click-to-edit mode.
///
/// Adding this variant is serde-compatible with configs saved before it
/// existed: `#[serde(rename_all = "lowercase")]` only affects how a *present*
/// value is read/written, so old configs (which simply never mention
/// "livemarkdown") continue to deserialize into `Raw`/`Rendered`/`Split` as
/// before.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize, Default)]
#[serde(rename_all = "lowercase")]
pub enum ViewMode {
    ...
    Split,
    /// Live inline (Typora-style) WYSIWYG markdown: one continuous surface,
    /// markers hidden except on the cursor's line.
    LiveMarkdown,
}

impl ViewMode {
    /// Cycle through view modes: Raw → Split → Rendered → LiveMarkdown → Raw
    pub fn toggle(&self) -> Self {
        match self {
            ViewMode::Raw => ViewMode::Split,
            ViewMode::Split => ViewMode::Rendered,
            ViewMode::Rendered => ViewMode::LiveMarkdown,
            ViewMode::LiveMarkdown => ViewMode::Raw,
        }
    }
    ...
    pub fn label(&self) -> &'static str {
        match self {
            ...
            ViewMode::LiveMarkdown => "Live",
        }
    }

    /// Get an icon/symbol for the mode.
    pub fn icon(&self) -> &'static str {
        use crate::ui::phosphor_icons::{COLUMNS, EYE, NOTE_PENCIL, PENCIL_LINE};
        match self {
            ViewMode::Raw => NOTE_PENCIL,
            ViewMode::Rendered => EYE,
            ViewMode::Split => COLUMNS,
            ViewMode::LiveMarkdown => PENCIL_LINE,
        }
    }

    /// Check if this mode shows the raw editor.
    ///
    /// `LiveMarkdown` is `true` here: it renders through the same raw-buffer
    /// `FerriteEditor`/`EditorWidget` path as `Raw`, just with live inline
    /// styling layered on top — it is not the block-based `Rendered` widget.
    pub fn shows_raw(&self) -> bool {
        matches!(self, ViewMode::Raw | ViewMode::Split | ViewMode::LiveMarkdown)
    }
    ...
    pub fn description(&self) -> &'static str {
        match self {
            ...
            ViewMode::LiveMarkdown => "Live inline styled markdown (Typora-style)",
        }
    }
}
...
fn default_line_height() -> f32 {
    crate::theme::typescale::DEFAULT_BODY_LINE_HEIGHT
}
...
#[serde(default)]
pub struct Settings {
    /// Schema version of the on-disk config, for one-time migrations.
    ///
    /// Absent in configs written before migrations existed, which `serde`
    /// resolves to 0 — exactly the "needs migrating" case.
    #[serde(default)]
    ...
    /// Line height for editor body text, as a multiple of the font size.
    ///
    /// Pinned rather than left to the font's own metrics: Inter reports ~1.20
    /// and Literata ~1.49, so relying on native leading would silently change
    /// the reading rhythm whenever the user switched typeface. Higher values
    /// are easier to read across a wide measure but show fewer lines at once.
...
/// Accent shipped before the warm palette — a cool blue tuned for dark chrome.
const LEGACY_ACCENT_RGB: [u8; 3] = [100, 180, 255];

/// Syntax theme shipped as the hardcoded default before it followed the UI theme.
const LEGACY_SYNTAX_THEME: &str = "base16-ocean.dark";

impl Settings {
    /// Bring a config loaded from disk up to [`CURRENT_CONFIG_VERSION`].
    ///
    /// Returns `true` if anything changed, so the caller can persist the result.
    ///
    /// Changing a `Default` value does nothing for anyone who already has a
    /// config on disk — the saved value always wins. Without this, every
    /// existing user stays on the old palette and the old syntax theme forever.
    ///
    /// Each step only rewrites a value that still **equals the old default**, so
    /// a setting the user deliberately chose is left alone. The version stamp
    /// then makes it a one-time operation: a user who later picks the blue
    /// accent on purpose keeps it.
    pub fn migrate(&mut self) -> bool {
        if self.config_version >= CURRENT_CONFIG_VERSION {
            return false;
        }

        // v0 → v1: the warm palette. Both of these were defaults that measured
        // badly on a light background — the blue accent gave 2.2:1 headings,
        // and the dark syntax theme painted #c0c5ce body text on a light page
        // for 1.6:1. Empty means "follow the UI theme".
        if self.accent_color == LEGACY_ACCENT_RGB {
            self.accent_color = crate::theme::accent::DEFAULT_ACCENT_RGB;
        }
        if self.syntax_theme == LEGACY_SYNTAX_THEME {
            self.syntax_theme = String::new();
        }
...
impl Default for Settings {
    fn default() -> Self {
        Self {
            ...
            // Syntax Highlighting
            //
            // Empty means "Auto": follow the UI theme, picking
            // `DEFAULT_LIGHT_THEME` or `DEFAULT_DARK_THEME` per
            // `markdown::syntax::highlight_code_for_mode`. This previously
            // hardcoded "base16-ocean.dark", so a light UI got a dark theme's
            // foreground (#c0c5ce) painted on a light page — body text in Raw
            // mode measured 1.6:1. The settings UI already renders empty as
            // "Auto", so Auto was always the intent.
            ...
            // Maximum Line Width Settings
            max_line_width: MaxLineWidth::default(), // Col80 — the reading measure
...
impl Settings {
    ...
    pub fn sanitize(&mut self) {
        ...
        // Clamp line height
        self.line_height = self.line_height.clamp(
...
mod tests {
    ...
    #[test]
    fn test_default_font_family_is_literata() {
        assert_eq!(EditorFont::default(), EditorFont::Literata);
        assert_eq!(Settings::default().font_family, EditorFont::Literata);
    }

    #[test]
    fn test_editor_font_serde_round_trip() {
        for font in [
            EditorFont::Literata,
            EditorFont::Inter,
            EditorFont::JetBrainsMono,
        ] {
            let json = serde_json::to_string(&font).expect("serialize EditorFont");
            let back: EditorFont = serde_json::from_str(&json).expect("deserialize EditorFont");
            assert_eq!(back, font);
        }
    }

    #[test]
    fn test_editor_font_deserializes_legacy_names() {
        // Old configs written before Literata became the default must still load.
        let inter: EditorFont = serde_json::from_str("\"inter\"").expect("legacy inter");
        assert_eq!(inter, EditorFont::Inter);
        let jetbrains: EditorFont =
            serde_json::from_str("\"jetbrainsmono\"").expect("legacy jetbrainsmono");
        assert_eq!(jetbrains, EditorFont::JetBrainsMono);
    }
    ...
    fn test_view_mode_toggle() {
        // Raw → Split → Rendered → LiveMarkdown → Raw
        assert_eq!(ViewMode::Raw.toggle(), ViewMode::Split);
        assert_eq!(ViewMode::Split.toggle(), ViewMode::Rendered);
        assert_eq!(ViewMode::Rendered.toggle(), ViewMode::LiveMarkdown);
        assert_eq!(ViewMode::LiveMarkdown.toggle(), ViewMode::Raw);
    }
    ...
    fn test_view_mode_all() {
        ...
        assert!(all.contains(&ViewMode::LiveMarkdown));
    }
    ...
    fn test_settings_deserialize_empty_json() {
        ...
        // `config_version` deliberately defaults to 0 on deserialize rather
        // than to CURRENT_CONFIG_VERSION: an absent version is exactly how a
        // pre-migration config looks, and it has to be recognised as such.
        // `Settings::default()` is already current, so compare after migrating.
    ...
    /// An old config still carrying the legacy defaults must be moved onto the
    /// new ones — otherwise changing `Default` helps nobody who already ran the
    /// app.
    #[test]
    fn test_migrate_replaces_legacy_defaults() {
        let mut s = Settings {
            config_version: 0,
            accent_color: LEGACY_ACCENT_RGB,
            syntax_theme: LEGACY_SYNTAX_THEME.to_string(),
            ..Settings::default()
        };
        assert!(s.migrate(), "migration should report a change");
        assert_eq!(s.accent_color, crate::theme::accent::DEFAULT_ACCENT_RGB);
        assert!(s.syntax_theme.is_empty(), "syntax theme should follow the UI theme");
        assert_eq!(s.config_version, CURRENT_CONFIG_VERSION);
    }

    /// The important half: a value the user actually chose must survive, even
    /// when it is not the new default.
    #[test]
    fn test_migrate_preserves_deliberate_choices() {
        let chosen_accent = [10, 200, 40];
        let chosen_theme = "Solarized (dark)";
        let mut s = Settings {
            config_version: 0,
            accent_color: chosen_accent,
            syntax_theme: chosen_theme.to_string(),
            ..Settings::default()
        };
        s.migrate();
        assert_eq!(s.accent_color, chosen_accent);
        assert_eq!(s.syntax_theme, chosen_theme);
        assert_eq!(s.config_version, CURRENT_CONFIG_VERSION);
    }

    /// Migration runs once. A user who later picks the old blue on purpose
    /// keeps it.
    #[test]
    fn test_migrate_is_idempotent_and_runs_once() {
        let mut s = Settings {
            config_version: 0,
            accent_color: LEGACY_ACCENT_RGB,
            ..Settings::default()
        ...
        assert!(s.migrate());
        assert!(!s.migrate(), "second run must be a no-op");

        // User deliberately re-picks the legacy blue afterwards.
        s.accent_color = LEGACY_ACCENT_RGB;
        assert!(!s.migrate(), "must not re-migrate an up-to-date config");
        assert_eq!(s.accent_color, LEGACY_ACCENT_RGB, "choice must survive");
    }

    /// A config written before migrations existed has no version field at all.
    #[test]
    fn test_config_without_version_field_parses_as_zero() {
        let json = r#"{"theme": "dark"}"#;
        let s: Settings = serde_json::from_str(json).unwrap();
        assert_eq!(s.config_version, 0, "absent version must mean 'needs migrating'");
    }

    #[test]
    fn test_line_height_default_and_clamping() {
        assert_eq!(
            Settings::default().line_height,
            crate::theme::typescale::DEFAULT_BODY_LINE_HEIGHT
        );
        ...
        s.line_height = 0.2;
        s.sanitize();
        assert_eq!(s.line_height, crate::theme::typescale::MIN_LINE_HEIGHT);

        s.line_height = 9.0;
        s.sanitize();
        assert_eq!(s.line_height, crate::theme::typescale::MAX_LINE_HEIGHT);
    }

    /// A config written before line height was configurable must still load.
    #[test]
    fn test_line_height_absent_from_old_config() {
        let json = r#"{"theme": "dark"}"#;
        let s: Settings = serde_json::from_str(json).unwrap();
        assert_eq!(
            s.line_height,
    ...
    fn test_max_line_width_default() {
        assert_eq!(MaxLineWidth::default(), MaxLineWidth::Col80);
    }
    ...
    fn test_settings_max_line_width_default() {
        let settings = Settings::default();
        assert_eq!(settings.max_line_width, MaxLineWidth::Col80);
    }

    #[test]
    fn test_settings_backward_compatibility_max_line_width() {
        // Old JSON without max_line_width field should use the current default.
        // Note this deliberately changes what pre-existing configs resolve to:
        // a saved config that never mentioned max_line_width now reads as Col80.
        let json = r#"{"theme": "dark"}"#;
        let settings: Settings = serde_json::from_str(json).unwrap();
        assert_eq!(settings.max_line_width, MaxLineWidth::Col80);
    }
