...
///
/// Combines [`detect_block_formatting_state`] (line-level and fenced-code
/// detection — expensive, safe to cache per line) with
/// [`apply_inline_formatting_state`] (delimiter detection — cheap, but
/// column-dependent so callers that cache should recompute this part fresh).
/// See `Tab::formatting_state_at` for the caching split.
pub fn detect_raw_formatting_state(text: &str, cursor: usize) -> FormattingState {
    let mut state = detect_block_formatting_state(text, cursor);
    apply_inline_formatting_state(text, cursor, &mut state);
    state
}

/// Detect the block-level formatting state (heading, blockquote, list, fenced
/// code block) at a cursor position.
///
/// This is the expensive half of formatting detection: [`is_inside_code_block`]
/// scans the document from byte 0 to `cursor`. It depends only on which line
/// the cursor is on, not the column, so callers may cache the result keyed on
/// the cursor's line number.
pub fn detect_block_formatting_state(text: &str, cursor: usize) -> FormattingState {
...
/// Fill in the inline formatting fields (bold, italic, inline code,
/// strikethrough) on an already-populated block-level [`FormattingState`].
///
/// Unlike the block-level portion, this is column-dependent and must be
/// recomputed on every call, but it is bounded to the current line (via
/// [`has_balanced_inline_marker`]) so that is cheap even on large documents.
/// `state.is_code_block` must already be set (see [`detect_block_formatting_state`]).
pub fn apply_inline_formatting_state(text: &str, cursor: usize, state: &mut FormattingState) {
    // Adjust cursor to UTF-8 char boundary
    let cursor = floor_char_boundary(text, cursor.min(text.len()));
