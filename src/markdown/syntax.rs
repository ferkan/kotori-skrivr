...
impl SyntaxHighlighter {
    ...
    pub fn find_syntax_for_language(
        &self,
        language: &str,
...
mod tests {
    ...
    fn test_powershell_syntax_available() {
        ...
        assert!(
            ps1_syntax.is_some(),
            "PowerShell syntax should be found for 'ps1'"
        );

        // Also check by name
        let powershell_syntax = highlighter.find_syntax_for_language("powershell");
        assert!(
            powershell_syntax.is_some(),
            "PowerShell syntax should be found for 'powershell'"
        );
    ...
    fn test_powershell_highlighting() {
        ...
        assert!(
            total_segments > 2,
            "PowerShell should have syntax-highlighted segments, got {}",
            total_segments
        );
        ...
        assert_eq!(
            full_text, code,
            "Content must be preserved after highlighting"
        );
    }
