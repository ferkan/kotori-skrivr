...
///
/// Delegates to `Tab::formatting_state_at`, which caches the block-level
/// portion (heading/list/blockquote/code-fence) keyed on the cursor's line so
/// column-only movement does not re-scan the document.
pub(crate) fn get_formatting_state_for(
    tab: &mut Tab,
    cursor_line: usize,
    cursor_col: usize,
) -> FormattingState {
    let char_index = line_col_to_char_index(&tab.content, cursor_line, cursor_col);
    tab.formatting_state_at(char_index, cursor_line)
}
