...
impl FerriteApp {
    ...
    /// Apply a formatting command directly to the active tab's source text.
    ///
    /// **Never touches `FerriteEditor`.** This is the path for Rendered mode,
    /// where routing through the editor is actively wrong:
    ///
    /// * A `FerriteEditor` for the tab usually still exists in egui memory from
    ///   a previous visit to Raw or LiveMarkdown mode, so
    ///   [`Self::handle_format_command_with_selection`] finds one and applies
    ///   the command to *its* buffer — which rendered-mode edits never wrote to.
    /// * Worse, when that apply reports "not applied" it falls through to
    ///   [`Self::handle_format_command`], which re-reads the hidden editor's own
    ///   stale caret. The visible symptom is formatting landing on whatever line
    ///   that caret was last parked on rather than the user's selection.
    ///
    /// `selection` is a char range into `tab.content` and must be non-empty —
    /// Rendered mode reports no caret position of its own, so there is no
    /// meaningful insertion point to fall back to.
    pub(crate) fn handle_format_command_on_source(
        &mut self,
        cmd: MarkdownFormatCommand,
        ...
        let content = tab.content.clone();
        let result = apply_raw_format(&content, Some(selection), cmd);

        if !result.applied {
            debug!("Rendered-mode formatting not applied for {:?}", cmd);
            return;
        }

        tab.set_content(result.text.clone());
        // The rendered view rebuilds from source; tell any cached editor state
        // that the buffer moved underneath it.
        tab.notify_external_content_change();
        ...
        debug!("Applied rendered-mode formatting via source: {:?}", cmd);
    }
