        ...
        egui::Panel::top("title_bar")
            ...
            .show_inside(ui, |ui| {
                ...
                ui.horizontal_centered(|ui| {
                    ...
                    // - ViewModeSegment: labelled segments are measured at runtime and capped
                    //   at `MAX_LABELED_TOTAL_WIDTH` (200px, see `view_segment.rs`); beyond
                    //   that the control falls back to fixed-width icon glyphs
                    //   (4 x 26px = 104px, or 2 x 26px = 52px for the 2-mode variant).
                    //
                    // This must cover the WORST case, not the typical one: if the
                    // reservation is too small the drag region overlaps the buttons and
                    // swallows clicks meant for them, which is the exact Linux hit-testing
                    // bug this exists to prevent. 228px fixed chrome + the segment
                    // control's own 200px ceiling = 428px, plus a little slack.
                    //
                    // Over-reserving is harmless — it only shortens the draggable strip,
                    // and there is far more title bar than the drag gesture needs.
                    const WINDOW_BUTTON_AREA_WIDTH: f32 = 440.0;
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        ...
                        let close_label = t!("a11y.close_button").to_string();
                        crate::ui::a11y::name_widget(&close_btn, &close_label, true);
                        crate::ui::a11y::focus_ring(ui, &close_btn, 4);
                        close_btn.on_hover_text(close_label);
                        ...
                        let max_tooltip = if is_maximized {
                            t!("a11y.restore_button").to_string()
                        } else {
                            t!("a11y.maximize_button").to_string()
                        };
                        crate::ui::a11y::name_widget(&max_btn, &max_tooltip, true);
                        crate::ui::a11y::focus_ring(ui, &max_btn, 4);
                        ...
                        let min_label = t!("a11y.minimize_button").to_string();
                        crate::ui::a11y::name_widget(&min_btn, &min_label, true);
                        crate::ui::a11y::focus_ring(ui, &min_btn, 4);
                        min_btn.on_hover_text(min_label);
                        ...
                        let fs_label = fs_tooltip.to_string();
                        crate::ui::a11y::name_widget(&fullscreen_btn, &fs_label, true);
                        crate::ui::a11y::focus_ring(ui, &fullscreen_btn, 4);
                        fullscreen_btn.on_hover_text(fs_label);
                ...
                if let Some(view_action) = title_bar_view_action {
                    if let Some(tab) = self.state.active_tab_mut() {
                        let new_mode = match view_action {
                            ...
                            ViewSegmentAction::SetLiveMarkdown => ViewMode::LiveMarkdown,
                        };
