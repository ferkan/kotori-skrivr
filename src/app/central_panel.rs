...
use crate::ui::{set_overlay_blocks_nav_buttons, FileOperationResult, GoToLineResult};
use eframe::egui;
...
impl FerriteApp {
    ...
    ///
    /// Formatting/toolbar actions used to be deferred and returned from here;
    /// that toolbar now lives at the app level (see `app/mod.rs`), so this
    /// always returns `None`. The `Option<DeferredFormatAction>` return type
    /// stays for call-site compatibility rather than a one-off signature change.
    pub(crate) fn render_central_panel(
        ...
        let deferred_format_action: Option<DeferredFormatAction> = None;
        ...
        egui::CentralPanel::default()
            ...
            if let TabKind::Special(special_kind) = active_tab_kind {
                ...
                {
                    match view_mode {
                        // `LiveMarkdown` renders through the same raw-buffer
                        // `EditorWidget`/`FerriteEditor` path as `Raw` — it is
                        // not the block-based `Rendered` widget. The only
                        // difference is the `.live_markdown(...)` flag passed
                        // to `EditorWidget` below, which the widget only
                        // honors when the tab is actually markdown.
                        ViewMode::Raw | ViewMode::LiveMarkdown => {
                            ...
                            // Formatting/toolbar actions are now handled by the merged
                            // file/format bar rendered once at the app level
                            // (`render_title_bar`'s sibling panel in `app/mod.rs`), not
                            // per view-mode branch here.
                            ...
                            if let Some(tab) = self.state.active_tab_mut() {
                                ...
                                // Calculate layout for editor and minimap (no format bar
                                // reservation — that chrome now lives above the tab bar).
                                ...
                                let mut editor = EditorWidget::new(tab)
                                    ...
                                    .vim_mode(vim_mode)
                                    .live_markdown(
                                        view_mode == ViewMode::LiveMarkdown && is_markdown_file
                                    )
                                ...
                                tab.last_scroll_anchor = (
                                    editor_output.scroll_anchor_line,
                        ...
                        ViewMode::Split => {
                            ...
                            if structured_type.is_some() {
                                ...
                                // The format/file bar no longer reserves space here — it
                                // is rendered once, full-width, above the tab bar.
                                ...
                                if let Some(tab) = self.state.active_tab_mut() {
                                    ...
                                    editor_scroll_anchor_fraction =
                                        editor_output.scroll_anchor_fraction;
                                    tab.last_scroll_anchor =
                                        (editor_scroll_anchor_line, editor_scroll_anchor_fraction);
                        ...
                        ViewMode::Rendered => {
                            ...
                            if let Some(file_type) = tabular_type {
                                ...
                                // The merged file/format bar is rendered once at the app
                                // level (see `app/mod.rs`), not per view-mode branch. Its
                                // Rendered-mode formatting actions are applied directly via
                                // `handle_format_command_on_source` there (Rendered mode has
                                // no FerriteEditor to defer through).

                                if let Some(tab) = self.state.active_tab_mut() {
                                    tab.prepare_undo_snapshot_hashed();

                                    if !sync_on {
                                        // `pending_scroll_anchor` also carries the mode-switch
                                        // reading-position handoff (`Tab::set_view_mode` /
                                        // `toggle_view_mode`), which is unrelated to split-view
                                        // sync scrolling — preserve it across the clear below.
                                        let mode_switch_anchor = tab.pending_scroll_anchor;
                                        tab.clear_sync_pending_scroll();
                                        tab.pending_scroll_anchor = mode_switch_anchor;
                                    }
                                    ...
                                    // Track reading position (source line + fraction) so a view-mode
                                    // switch away from Rendered can hand it to the incoming view.
                                    if !editor_output.line_mappings.is_empty() {
                                        tab.last_scroll_anchor =
                                            SyncScrollState::preview_y_to_source_anchor(
                                                editor_output.scroll_offset,
                                                &editor_output.line_mappings,
                                            );
                                    }

                                    // Apply a pending scroll anchor handed off from another view mode
                                    // (see `Tab::set_view_mode`/`toggle_view_mode`). The line→Y mapping
                                    // requires `line_mappings` from at least one render pass, so if they
                                    // are empty (first frame after the switch) leave the anchor pending
                                    // and retry next frame rather than silently dropping it.
                                    if let Some((line, frac)) = tab.pending_scroll_anchor {
                                        if !editor_output.line_mappings.is_empty() {
                                            let total_lines = tab.content.lines().count();
                                            let clamped_line = crate::state::Tab
                                                ::clamp_scroll_anchor_line(line, total_lines);
                                            let target_y =
                                                SyncScrollState::source_anchor_to_preview_y(
                                                    clamped_line,
                                                    frac,
                                            ...
                                            tab.pending_scroll_anchor = None;
                                            ui.ctx().request_repaint();
                                        }
                                    }
