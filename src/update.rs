//! Update checking module for Kotori Skrivr.
//!
...
/// Whether this build has a release feed to check against.
///
/// Set to `true` and point the two constants below at the Kotori Skrivr
/// repository once releases are published. See [`spawn_update_check`] for why
/// this is gated rather than left pointing at the upstream feed.
const UPDATE_FEED_CONFIGURED: bool = false;

const GITHUB_API: &str = "https://api.github.com/repos/OlaProeis/Kotori Skrivr/releases/latest";
const GITHUB_RELEASES_PREFIX: &str = "https://github.com/OlaProeis/Kotori Skrivr/releases/";
...
fn check_for_update_blocking() -> UpdateCheckResult {
    let response = match ureq::get(GITHUB_API)
        .set("User-Agent", &format!("KotoriSkrivr/{}", CURRENT_VERSION))
...
pub fn spawn_update_check() -> mpsc::Receiver<UpdateCheckResult> {
    ...
    // Kotori Skrivr has no release feed of its own yet.
    //
    // The machinery below still points at the upstream Kotori Skrivr repository it
    // was inherited from. Querying it would compare *our* version against
    // *their* releases and, whenever upstream is ahead, tell the user an
    // update is available and send them to download a different program.
    // Reporting "not configured" is the honest answer until a Kotori Skrivr
    // release feed exists; at that point, set the constants above and delete
    // this short-circuit.
    if !UPDATE_FEED_CONFIGURED {
        let _ = tx.send(UpdateCheckResult::Error(
            t!("settings.about.update_check_unavailable").to_string(),
        ));
        return rx;
    }
...
mod tests {
    ...
    fn test_github_url_validation() {
        ...
        let valid = "https://github.com/OlaProeis/Kotori Skrivr/releases/tag/v0.2.7";
        ...
        let spoof = "https://github.com.evil.com/OlaProeis/Kotori Skrivr/releases/tag/v1.0";
