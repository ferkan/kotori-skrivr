...
pub struct WrapInfo {
    ...
    /// INCLUDING any blank space rendered above the line (see `space_above`).
    pub height: f32,
    /// Extra blank space rendered above this line's text, in pixels (e.g. heading margins
    /// in live markdown mode). Included in `height`. Zero for ordinary lines.
    pub space_above: f32,
}
...
impl ViewState {
    ...
    /// * `height` - Total height of the wrapped line in pixels, including `space_above`
    /// * `space_above` - Blank space rendered above the line's text (e.g. heading margins),
    ///   already included in `height`. Zero for ordinary lines.
    ///
    /// # Large File Optimization
    ...
    pub fn set_line_wrap_info(&mut self, line: usize, visual_rows: usize, height: f32, space_above: f32) {
        ...
        let new_info = WrapInfo {
            ...
            space_above: space_above.max(0.0),
        };
    ...
    /// Returns the blank space rendered above a specific logical line's text
    /// (e.g. heading margins in live markdown mode).
    ///
    /// Returns 0.0 in uniform-heights mode or when the line has no wrap info.
    #[must_use]
    pub fn get_line_space_above(&self, line: usize) -> f32 {
        if self.use_uniform_heights {
            return 0.0;
        }
...
mod tests {
    ...
    fn test_visible_line_range_with_wrap_info() {
        ...
        for i in 0..5 {
            view.set_line_wrap_info(i, 2, 40.0, 0.0);
        }
        for i in 5..10 {
            view.set_line_wrap_info(i, 1, 20.0, 0.0);
        }
    ...
    fn test_disable_wrap() {
        let mut view = ViewState::new();
        view.enable_wrap(400.0);
        view.set_line_wrap_info(0, 3, 60.0, 0.0);
    ...
    /// The render loop and mouse hit-testing derive their y-coordinates
    /// independently: rendering stores the TEXT top
    /// (`row_top + space_above`), while `pos_to_cursor` accumulates row
    /// heights and subtracts `space_above` to get the offset inside a galley.
    /// If those two ever disagree, clicks land at the wrong point in the line
    /// — silently, and worse the further down the document you go.
    ///
    /// This pins the shared arithmetic: a click exactly at a line's text top
    /// must resolve to offset 0 within that line.
    #[test]
    fn text_top_and_hit_test_offset_agree_across_spaced_lines() {
        let mut view = ViewState::new();
        // A document mixing plain rows with heading rows that carry space.
        let rows = [(20.0, 0.0), (48.0, 28.0), (20.0, 0.0), (40.0, 20.0), (20.0, 0.0)];
        for (i, (height, space)) in rows.iter().enumerate() {
            view.set_line_wrap_info(i, 1, *height, *space);
        }

        let mut row_top = 0.0_f32;
        for (i, (height, space)) in rows.iter().enumerate() {
            assert_eq!(
                view.get_line_height(i),
                *height,
                "line {i}: recorded height must be the full row"
            );
            assert_eq!(view.get_line_space_above(i), *space, "line {i}: space");

            // What the render loop paints text at.
            let text_top = row_top + view.get_line_space_above(i);

            // What `pos_to_cursor` computes for a click at that same y.
            let y_in_line = (text_top - row_top - view.get_line_space_above(i)).max(0.0);
            assert_eq!(y_in_line, 0.0, "line {i}: click at text top must be offset 0");

            // A click one pixel into the text is one pixel into the galley.
            let y_in_line = (text_top + 1.0 - row_top - view.get_line_space_above(i)).max(0.0);
            assert_eq!(y_in_line, 1.0, "line {i}: offsets must track 1:1 inside text");

            row_top += view.get_line_height(i);
        }

        // And the accumulated total is unchanged by where the space sits.
        let expected: f32 = rows.iter().map(|(h, _)| h).sum();
        assert_eq!(row_top, expected, "row tops must tile contiguously");
    }

    #[test]
    fn test_get_line_height_includes_space_above() {
        let mut view = ViewState::new();
        view.set_line_height(20.0);

        // A heading's total height (60px) already includes the 20px of
        // blank space rendered above its text, per `set_line_wrap_info`'s
        // contract: `height` is the TOTAL row height.
        view.set_line_wrap_info(0, 1, 60.0, 20.0);

        assert_eq!(view.get_line_height(0), 60.0);
    }

    #[test]
    fn test_get_line_space_above() {
        let mut view = ViewState::new();
        view.set_line_height(20.0);
        view.set_line_wrap_info(0, 1, 60.0, 20.0);
        view.set_line_wrap_info(1, 1, 20.0, 0.0);

        assert_eq!(view.get_line_space_above(0), 20.0);
        assert_eq!(view.get_line_space_above(1), 0.0);
    }

    #[test]
    fn test_get_line_space_above_no_wrap_info() {
        let view = ViewState::new();
        assert_eq!(view.get_line_space_above(0), 0.0);
        assert_eq!(view.get_line_space_above(100), 0.0);
    }

    #[test]
    fn test_get_line_space_above_uniform_heights_mode() {
        let mut view = ViewState::new();
        view.set_line_height(20.0);
        view.configure_for_file_size(LARGE_FILE_THRESHOLD);
        assert!(view.uses_uniform_heights());

        // Uniform mode ignores wrap info entirely (set_line_wrap_info is a
        // no-op), so space above is always 0.0.
        view.set_line_wrap_info(0, 1, 60.0, 20.0);
        assert_eq!(view.get_line_space_above(0), 0.0);
    }
    ...
    fn test_rebuild_height_cache() {
        ...
        view.set_line_wrap_info(0, 2, 40.0, 0.0); // Line 0: 2 rows, 40px
        view.set_line_wrap_info(1, 1, 20.0, 0.0); // Line 1: 1 row, 20px
        view.set_line_wrap_info(2, 3, 60.0, 0.0); // Line 2: 3 rows, 60px
    ...
    fn test_total_content_height() {
        ...
        view.set_line_wrap_info(0, 2, 40.0, 0.0);
        view.set_line_wrap_info(1, 1, 20.0, 0.0);
        view.set_line_wrap_info(2, 3, 60.0, 0.0);
        view.rebuild_height_cache(3);
