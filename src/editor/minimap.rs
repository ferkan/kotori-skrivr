...
mod tests {
    ...
    fn test_heading_level_color_semantic_dark() {
        let accent = crate::theme::accent::default_accent();
        let colors: Vec<_> = (1..=6)
            .map(|l| heading_level_color_semantic(l, true, accent))
            .collect();

        // H1 tracks the user's accent — that is the contract, not any
        // particular hue. Asserting "blue" here only asserted what the default
        // accent happened to be, and broke when the palette changed.
        assert_eq!(colors[0], accent);

        // H2 is a fixed green, independent of the accent.
        assert!(colors[1].g() > colors[1].r());
        assert!(colors[1].g() > colors[1].b());

        // The levels must be mutually distinguishable — this is what the test
        // always claimed to check and never did.
        for (i, a) in colors.iter().enumerate() {
            for (j, b) in colors.iter().enumerate().skip(i + 1) {
                assert_ne!(a, b, "H{} and H{} share a color", i + 1, j + 1);
            }
        }
    }
    ...
    fn test_heading_level_color_semantic_light() {
        let accent = crate::theme::accent::default_accent();
        let colors: Vec<_> = (1..=6)
            .map(|l| heading_level_color_semantic(l, false, accent))
            .collect();

        // H1 is the accent, darkened slightly for the light ground — so it must
        // stay recognisably the accent's hue without being identical to it.
        assert_ne!(colors[0], accent, "light H1 should be darkened");
        assert_eq!(
            colors[0],
            crate::theme::accent::lerp_color(accent, Color32::BLACK, 0.08)
        );
