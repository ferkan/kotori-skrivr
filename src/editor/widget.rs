...
pub struct EditorWidget<'a> {
    ...
    font_size: f32,
    /// Body line-height multiplier for the editor (`Settings::line_height`).
    line_height: f32,
    ...
    vim_mode: bool,
    /// Whether the live inline (Typora-style) markdown mode is active for
    /// this tab (`ViewMode::LiveMarkdown` and the file is markdown). See
...
impl<'a> EditorWidget<'a> {
    ...
    pub fn new(tab: &'a mut Tab) -> Self {
        Self {
            tab,
            font_size: 14.0,
            line_height: typescale::DEFAULT_BODY_LINE_HEIGHT,
            word_wrap: true,
            id: None,
            show_line_numbers: true,
            theme_colors: None,
    ...
    ///
    /// Pinned explicitly rather than left to the font: Inter reports ~1.20
    ...
    pub fn line_height(mut self, line_height: f32) -> Self {
        self.line_height = line_height.clamp(typescale::MIN_LINE_HEIGHT, typescale::MAX_LINE_HEIGHT);
        self
    }
    ...
    /// Set whether the live inline (Typora-style) markdown mode is active.
    /// The widget still gates this on the file actually being markdown (see
    /// `show()`), so passing `true` for a non-markdown file is a no-op.
    #[must_use]
    pub fn live_markdown(mut self, enabled: bool) -> Self {
        self.live_markdown = enabled;
        self
    }
    ...
    pub fn show(mut self, ui: &mut Ui) -> EditorOutput {
        ...
        //
        // The average character advance differs by roughly 20% between
        // monospace (~0.6em) and proportional (~0.5em) families. Using the
        // monospace figure for both made `Col80` render at ~95 proportional
        // characters, well past a comfortable reading measure.
        let char_width = self.font_size * if self.font_family.is_monospace() { 0.6 } else { 0.5 };
        ...
        // Centre the text column whenever a maximum width is set.
        //
        // This used to be gated on zen mode, so the default experience put an
        // unbounded line length against the left edge of the window — on a wide
        // display that is 200+ characters per line. A measure-limited, centred
        // column is what makes the surface read as a page rather than as text
        // sprayed across a void, and it should not require entering a mode.
        let available_width = ui.available_width();
        let content_offset_x = match max_wrap_width_px {
            Some(max_width) => {
                let margin = (available_width - max_width).max(0.0) / 2.0;
                debug!(
                    "Centering text column: available={:.1}, max_width={:.1}, margin={:.1}",
        ...
        // Configure live inline markdown mode. Gated here (not just by the
        // caller) on the file actually being markdown, so a stray
        // `.live_markdown(true)` on a non-markdown tab is harmless.
        let is_markdown_file = self
            .file_path
            .as_ref()
            .map(|p| crate::state::FileType::from_path(p).is_markdown())
            .unwrap_or(false);
        editor.set_live_markdown_enabled(self.live_markdown && is_markdown_file);
        ...
        if let Some((line, fraction)) = self.tab.pending_scroll_anchor.take() {
            let total_lines = editor.buffer().line_count();
            let line_0 = Tab::clamp_scroll_anchor_line(line, total_lines).saturating_sub(1);
