...
pub(crate) fn visuals_from_palette(colors: &ThemeColors) -> Visuals {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // NOTE: mirrors the constraint documented in `light.rs` — `active.fg_stroke.color`
    // is also what `Visuals::strong_text_color()` returns for `RichText::strong()`,
    // so it has to work on the panel background as well as on `active.bg_fill`.
    // In dark mode those pull in opposite directions: the raw accent is a light
    // blue, so white on it measures only 2.21:1, but the near-black that would
    // fix that drops `.strong()` labels to 1.11:1 on the dark panel — invisible.
    // Resolved by darkening the fill instead of the foreground: `base.selected`
    // gives 6.90:1 against white while `.strong()` keeps its 16.67:1 on panels.
    visuals.widgets.active.bg_fill = colors.base.selected;
    visuals.widgets.active.weak_bg_fill = colors.base.selected;
    visuals.widgets.active.bg_stroke = Stroke::new(1.0, colors.ui.accent);
